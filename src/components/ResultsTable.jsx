...
function ResultsTable({ data, onDelete, onFlag }) {
  ...
  // Reset resubmit state when modal closes/opens
  useEffect(() => {
  ...
  // Allow Resubmit handler
  const handleAllowResubmit = async () => {
    if (!selectedResponse) return
    setAllowingResubmit(true)

    try {
      // Update status in Supabase to 'Resubmit Allowed'
      ...
      if (error) {
        console.error('Resubmit allow error:', error)
        alert('Failed to allow resubmit: ' + (error.message || 'Unknown error'))
      } else {
        // Remove the anti-duplicate localStorage key so the student can resubmit
        // Key format: rtu_submitted_[email]_[office]
        const email = selectedResponse.respondent_email || ''
        const office = selectedResponse.office || ''
        const localKey = `rtu_submitted_${email}_${office}`
        localStorage.removeItem(localKey)

        setResubmitDone(true)

        // Update the selectedResponse status locally so the badge updates
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button
                        ...
                        style={{
                          padding: '4px 8px',
                          backgroundColor: deletingId === item.id ? '#fee2e2' : 'transparent',
                          color: '#DC2626',
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: '#FFFFFF', borderRadius: '16px', padding: '32px', maxWidth: '600px', width: '90%', maxHeight: '80vh', overflow: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
            ...
            <button
              ...
              disabled={allowingResubmit || resubmitDone}
              style={{
                width: '100%',
                marginBottom: '10px',
                padding: '12px',
                backgroundColor: resubmitDone ? '#16A34A' : allowingResubmit ? '#93C5FD' : '#FFD700',
                color: resubmitDone ? '#FFFFFF' : '#1A1A2E',
                border: 'none',
                borderRadius: '8px',
                fontSize: '14px',
                fontWeight: '600',
                cursor: allowingResubmit || resubmitDone ? 'not-allowed' : 'pointer',
                transition: 'background-color 0.2s'
              }}
            >
              {resubmitDone
                ? '✅ Resubmit Allowed — Student Can Now Resubmit'
                : allowingResubmit
                ? '⏳ Processing...'
                : '🔄 Allow Resubmit'}
            </button>
