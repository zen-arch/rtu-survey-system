...
function StaffDashboard() {
  ...
  // ── Fetch pending resubmit requests for this office ──
  const fetchResubmitRequests = async () => {
    ...
    if (!error) setResubmitRequests(data || [])
  }

  useEffect(() => {
    fetchResubmitRequests()
    const interval = setInterval(fetchResubmitRequests, 30000)
  ...
  // ── Approve resubmit ──
  const handleApproveResubmit = async (req) => {
    ...
    if (!e1 && !e2) {
      setResubmitRequests(prev => prev.filter(r => r.id !== req.id))
      // Refresh responses so the status badge updates
      fetchResponses(selectedOffice)
    }
  }

  // ── Deny resubmit ──
  const handleDenyResubmit = async (req) => {
    const { error } = await supabase
      .from('resubmit_requests')
      .update({ status: 'denied' })
      .eq('id', req.id)
    if (!error) setResubmitRequests(prev => prev.filter(r => r.id !== req.id))
  }

  const pendingCount = resubmitRequests.length

  const formatResubmitTime = (iso) => {
    if (!iso) return ''
  ...
  const handleEditSurvey = (survey) => {
    ...
    try { setQuestions(JSON.parse(survey.questions) || []) } catch { setQuestions([]) }
  ...
  const addQuestion = (type) => {
    setQuestions(prev => [...prev, { id: `q_${Date.now()}`, type, text: '', options: type === 'multiple_choice' ? ['', ''] : [] }])
  }
  ...
  const handleSaveSurvey = async () => {
    ...
    const payload = { title: surveyTitle, status: surveyStatus, questions: JSON.stringify(questions), office: selectedOffice, updated_at: new Date().toISOString() }
    let error
    if (editingSurvey) {
    ...
    setTimeout(() => { setBuilderSuccess(''); setActiveView('surveys') }, 1500)
  }

  useEffect(() => {
    if (selectedOffice) { fetchResponses(selectedOffice); setCurrentPage(1) }
  }, [selectedOffice])
  ...
  const averageRating = responses.length > 0
    ? (responses.reduce((sum, r) => sum + parseFloat(r.average_rating || 0), 0) / responses.length).toFixed(2)
    : '0.00'
  ...
  const monthlyData = useMemo(() => {
    ...
    return Object.values(map).map(m => ({ month: m.month, avg: parseFloat((m.total / m.count).toFixed(2)) })).slice(-6)
  }, [responses])

  const clientTypeData = useMemo(() => {
    const map = {}
    responses.forEach(r => { const t = r.client_type || 'Unknown'; map[t] = (map[t] || 0) + 1 })
  ...
  const renderStars = (rating) => {
    ...
    return (
      <div style={{ display: 'flex', gap: '2px' }}>
        {[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= val ? '#FFD700' : '#E0E7FF', fontSize: '16px' }}>★</span>)}
      </div>
    )
  }
  ...
  // ── Bell dropdown (reusable in both desktop & mobile) ──
  const BellDropdown = ({ isMobile }) => (
    <div style={{
      position: isMobile ? 'fixed' : 'absolute',
      top: isMobile ? '56px' : '60px',
      ...(isMobile ? { left: '16px', right: '16px' } : { left: '274px' }),
      ...
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #E0E7FF', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ maxHeight: isMobile ? '60vh' : '380px', overflowY: 'auto' }}>
        {resubmitRequests.length === 0 ? (
          <div style={{ padding: '32px 16px', textAlign: 'center', color: '#6c757d', fontSize: '13px' }}>No pending requests</div>
        ) : (
          resubmitRequests.map(req => (
            <div key={req.id} style={{ padding: '12px 16px', borderBottom: '1px solid #F0F4FF', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                <div>
                  ...
                  <p style={{ margin: '2px 0 0', fontSize: '11px', color: '#adb5bd' }}>{formatResubmitTime(req.requested_at)}</p>
                </div>
                <span style={{ backgroundColor: '#FEF9C3', color: '#92400E', fontSize: '10px', fontWeight: 700, padding: '2px 8px', borderRadius: '10px', whiteSpace: 'nowrap', flexShrink: 0 }}>
                  Pending
                </span>
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  onClick={() => handleApproveResubmit(req)}
                  style={{ flex: 1, padding: '6px', backgroundColor: '#16A34A', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}
                >
                  <Check size={12} /> Approve
                </button>
                <button
                  onClick={() => handleDenyResubmit(req)}
                  style={{ flex: 1, padding: '6px', backgroundColor: '#FFFFFF', color: '#DC2626', border: '1px solid #DC2626', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}
                >
                  <Trash2 size={12} /> Deny
                </button>
              </div>
  ...
  const SidebarContent = ({ onNavClick }) => (
    <>
      <div style={{ padding: '0 24px 24px', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: '24px' }}>
        ...
        <div style={{ marginTop: '12px', backgroundColor: 'rgba(255,215,0,0.15)', borderRadius: '8px', padding: '8px 12px', border: '1px solid rgba(255,215,0,0.3)' }}>
          <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.6)', marginBottom: '2px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Managing Office</div>
        ...
        {navItems.map(item => (
          ...
          >
            ...
            {item.id === 'flagged' && flaggedCount > 0 && (
              <span style={{ marginLeft: 'auto', backgroundColor: '#DC2626', color: '#fff', borderRadius: '10px', padding: '2px 8px', fontSize: '11px', fontWeight: 700 }}>
                {flaggedCount}
              </span>
            )}
          </button>
      ...
      <div style={{ padding: '12px' }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', width: '100%', backgroundColor: 'transparent', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
        >
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#F5F7FA' }}>
      ...
      <aside
        ...
        style={{ width: '260px', backgroundColor: '#0033A0', position: 'fixed', height: '100vh', padding: '24px 0', boxShadow: '0 4px 16px rgba(0,0,0,0.15)', zIndex: 1000, display: 'flex', flexDirection: 'column' }}
      >
        <SidebarContent />

        {/* Bell — desktop */}
        <div ref={bellRef} style={{ position: 'absolute', top: '18px', right: '16px' }}>
          <button
            onClick={() => setBellOpen(o => !o)}
            style={{ position: 'relative', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', padding: '7px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            title="Resubmission Requests"
          >
            <Bell size={18} color="#FFFFFF" />
            {pendingCount > 0 && (
              <span style={{ position: 'absolute', top: '-4px', right: '-4px', backgroundColor: '#DC2626', color: '#fff', fontSize: '10px', fontWeight: 700, width: '16px', height: '16px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {pendingCount > 9 ? '9+' : pendingCount}
              </span>
            )}
          </button>
          {bellOpen && <BellDropdown isMobile={false} />}
        </div>
      </aside>
      ...
      <div
        ...
        style={{ display: 'none', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1100, backgroundColor: '#0033A0', padding: '10px 16px', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,51,160,0.3)' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <img src="/rtu_logo.png" alt="RTU" style={{ width: '32px', height: '32px', borderRadius: '50%' }} />
          <div>
            <div style={{ color: '#fff', fontWeight: 700, fontSize: '13px', lineHeight: 1.2 }}>RTU Survey System</div>
            <div style={{ color: '#FFD700', fontSize: '10px' }}>Staff Portal</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ position: 'relative' }}>
            <button
              onClick={() => setBellOpen(o => !o)}
              style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '8px', padding: '7px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
            >
              <Bell size={18} color="#FFFFFF" />
              {pendingCount > 0 && (
                <span style={{ position: 'absolute', top: '-3px', right: '-3px', backgroundColor: '#DC2626', color: '#fff', fontSize: '10px', fontWeight: 700, width: '16px', height: '16px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {pendingCount > 9 ? '9+' : pendingCount}
                </span>
              )}
            </button>
            {bellOpen && <BellDropdown isMobile={true} />}
          </div>
      ...
      {mobileOpen && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1200, display: 'flex' }}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ width: '260px', backgroundColor: '#0033A0', height: '100vh', padding: '24px 0', boxShadow: '4px 0 20px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', position: 'relative', animation: 'staffSlideIn 0.25s ease', overflowY: 'auto' }}
          >
            <button
              onClick={() => setMobileOpen(false)}
              style={{ position: 'absolute', top: '16px', right: '16px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', padding: '6px', cursor: 'pointer', display: 'flex' }}
            >
      ...
      {/* ══ MAIN CONTENT ══ */}
      <main className="staff-main-content" style={{ flex: 1, marginLeft: '260px', padding: '32px' }}>
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '28px' }}>
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 18px', backgroundColor: '#0033A0', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}
              >
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={clientTypeData} layout="vertical">
                      ...
                      <Bar dataKey="count" radius={[0,4,4,0]}>
                        {clientTypeData.map((entry, i) => <Cell key={i} fill={CLIENT_TYPE_COLORS[entry.type] || '#0033A0'} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
            ...
            <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', border: '1px solid #E0E7FF', overflow: 'hidden' }}>
              <div style={{ padding: '20px 24px', borderBottom: '1px solid #E0E7FF', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => setActiveView('responses')} style={{ fontSize: '13px', color: '#0033A0', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>View All →</button>
              </div>
              <ResponseTable responses={responses.slice(0, 5)} formatDate={formatDate} renderStars={renderStars} getStatusStyle={getStatusStyle} onSelect={setSelectedResponse} loading={loading} />
            </div>
          </>
        )}
        ...
        {activeView === 'responses' && (
          <>
            ...
            <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', border: '1px solid #E0E7FF', overflow: 'hidden' }}>
              <ResponseTable responses={paginatedResponses} formatDate={formatDate} renderStars={renderStars} getStatusStyle={getStatusStyle} onSelect={setSelectedResponse} loading={loading} />
              {totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderTop: '1px solid #E0E7FF' }}>
                  <span style={{ fontSize: '13px', color: '#6c757d' }}>Showing {(currentPage-1)*rowsPerPage+1}–{Math.min(currentPage*rowsPerPage, totalResponses)} of {totalResponses}</span>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => setCurrentPage(p => Math.max(1, p-1))} disabled={currentPage===1} style={{ padding: '8px 12px', border: '1px solid #E0E7FF', borderRadius: '6px', backgroundColor: '#fff', cursor: currentPage===1?'not-allowed':'pointer', opacity: currentPage===1?0.5:1 }}><ChevronLeft size={16} color="#6c757d" /></button>
                    <span style={{ padding: '8px 12px', fontSize: '13px', color: '#1A1A2E' }}>Page {currentPage} of {totalPages}</span>
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p+1))} disabled={currentPage===totalPages} style={{ padding: '8px 12px', border: '1px solid #E0E7FF', borderRadius: '6px', backgroundColor: '#fff', cursor: currentPage===totalPages?'not-allowed':'pointer', opacity: currentPage===totalPages?0.5:1 }}><ChevronRight size={16} color="#6c757d" /></button>
                  </div>
                </div>
              )}
            </div>
            ...
            {flaggedCount === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {responses.filter(r => r.status === 'Flagged').map(r => (
                  <FlaggedCard key={r.id} response={r} formatDate={formatDate} onResolved={(updated) => setResponses(prev => prev.map(x => x.id === updated.id ? updated : x))} />
                ))}
              </div>
            )}
          </>
        ...
        {activeView === 'surveys' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <button onClick={handleNewSurvey} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', backgroundColor: '#0033A0', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {loadingSurveys ? (
                ...
                <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '60px', textAlign: 'center', border: '1px solid #E0E7FF' }}>
                  ...
                  <button onClick={handleNewSurvey} style={{ padding: '10px 24px', backgroundColor: '#0033A0', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Create Survey</button>
                </div>
              ) : surveys.map(survey => {
                let qCount = 0
                try { qCount = JSON.parse(survey.questions)?.length || 0 } catch {}
                return (
                  <div key={survey.id} style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '24px', border: '1px solid #E0E7FF', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px', flexWrap: 'wrap' }}>
                        <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#1A1A2E', margin: 0 }}>{survey.title}</h3>
                        <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', backgroundColor: survey.status === 'published' ? '#16A34A' : '#F5F7FA', color: survey.status === 'published' ? '#fff' : '#6c757d', border: survey.status === 'published' ? 'none' : '1px solid #E0E7FF' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => handleEditSurvey(survey)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', backgroundColor: '#EEF2FF', color: '#0033A0', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                        <Edit3 size={15} /> Edit
                      </button>
                      <button onClick={() => handleDeleteSurvey(survey.id)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', backgroundColor: 'rgba(220,38,38,0.08)', color: '#DC2626', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
        ...
        {activeView === 'builder' && (
          <>
            ...
            {builderError && <div style={{ backgroundColor: 'rgba(220,38,38,0.08)', color: '#DC2626', padding: '12px 16px', borderRadius: '8px', marginBottom: '20px', fontSize: '14px', fontWeight: '500' }}>{builderError}</div>}
            {builderSuccess && <div style={{ backgroundColor: 'rgba(22,163,74,0.08)', color: '#16A34A', padding: '12px 16px', borderRadius: '8px', marginBottom: '20px', fontSize: '14px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}><CheckCircle size={16} /> {builderSuccess}</div>}
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '28px', border: '1px solid #E0E7FF', marginBottom: '20px' }}>
              ...
              <input type="text" placeholder="e.g. Cashier Office Satisfaction Survey" value={surveyTitle} onChange={e => setSurveyTitle(e.target.value)} style={{ width: '100%', padding: '12px 16px', fontSize: '15px', border: '2px solid #E0E7FF', borderRadius: '8px', outline: 'none', color: '#1A1A2E', backgroundColor: '#F5F7FA', boxSizing: 'border-box', marginBottom: '16px' }} />
              <label style={{ fontSize: '13px', fontWeight: '600', color: '#1A1A2E', display: 'block', marginBottom: '8px' }}>Status</label>
              <div style={{ display: 'flex', gap: '12px' }}>
                {['draft', 'published'].map(s => (
                  <button key={s} onClick={() => setSurveyStatus(s)} style={{ padding: '8px 20px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', border: '2px solid', backgroundColor: surveyStatus === s ? '#0033A0' : '#F5F7FA', color: surveyStatus === s ? '#fff' : '#6c757d', borderColor: surveyStatus === s ? '#0033A0' : '#E0E7FF' }}>
              ...
              {questions.map((q, qIndex) => (
                <div key={q.id} style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '24px', border: '2px solid #E0E7FF', position: 'relative' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    ...
                    <button onClick={() => removeQuestion(q.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#DC2626' }}><X size={18} /></button>
                  </div>
                  <input type="text" placeholder="Enter your question here..." value={q.text} onChange={e => updateQuestion(q.id, 'text', e.target.value)} style={{ width: '100%', padding: '10px 14px', fontSize: '14px', border: '2px solid #E0E7FF', borderRadius: '8px', outline: 'none', color: '#1A1A2E', backgroundColor: '#F5F7FA', boxSizing: 'border-box', marginBottom: q.type === 'multiple_choice' ? '12px' : 0 }} />
                  {q.type === 'multiple_choice' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {q.options.map((opt, optIndex) => (
                        <div key={optIndex} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                          <input type="text" placeholder={`Option ${optIndex + 1}`} value={opt} onChange={e => updateOption(q.id, optIndex, e.target.value)} style={{ flex: 1, padding: '8px 12px', fontSize: '13px', border: '1px solid #E0E7FF', borderRadius: '6px', outline: 'none', backgroundColor: '#F5F7FA' }} />
                          {q.options.length > 2 && <button onClick={() => removeOption(q.id, optIndex)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#DC2626' }}><X size={14} /></button>}
                        </div>
                      ))}
                      <button onClick={() => addOption(q.id)} style={{ alignSelf: 'flex-start', padding: '6px 14px', backgroundColor: '#EEF2FF', color: '#0033A0', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', marginTop: '4px' }}>+ Add Option</button>
                    </div>
                  )}
                  {q.type === 'rating' && (
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '20px', border: '1px solid #E0E7FF', marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                {[{ type: 'rating', label: '⭐ Rating (Stars)' }, { type: 'multiple_choice', label: '☑ Multiple Choice' }, { type: 'text', label: '✏️ Text Answer' }].map(btn => (
                  <button key={btn.type} onClick={() => addQuestion(btn.type)} style={{ padding: '10px 20px', backgroundColor: '#F5F7FA', color: '#0033A0', border: '2px solid #E0E7FF', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>{btn.label}</button>
                ))}
              </div>
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button onClick={() => setActiveView('surveys')} style={{ padding: '12px 24px', backgroundColor: '#F5F7FA', color: '#6c757d', border: '2px solid #E0E7FF', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleSaveSurvey} disabled={savingBuilder} style={{ padding: '12px 28px', backgroundColor: '#0033A0', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', opacity: savingBuilder ? 0.7 : 1 }}>
      ...
      {selectedResponse && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ backgroundColor: '#FFFFFF', borderRadius: '16px', padding: '32px', maxWidth: '560px', width: '90%', maxHeight: '80vh', overflow: 'auto', boxShadow: '0 16px 48px rgba(0,0,0,0.25)' }}>
              ...
              {(() => {
                  ...
                  try {
                    ...
                    return (d.questions || []).map((q, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #E0E7FF' }}>
                        ...
                        {q.type === 'rating' ? (
                          <div style={{ display: 'flex', gap: '2px' }}>{[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= (d.answers[q.id] || 0) ? '#FFD700' : '#E0E7FF', fontSize: '16px' }}>★</span>)}</div>
                        ) : (
                ...
                return [
                  ...
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #E0E7FF' }}>
                    ...
                    <div style={{ display: 'flex', gap: '2px' }}>{[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= (item.value || 0) ? '#FFD700' : '#E0E7FF', fontSize: '16px' }}>★</span>)}</div>
                  </div>
                ))
              })()}
            </div>
            ...
            <button onClick={() => setSelectedResponse(null)} style={{ width: '100%', padding: '12px', backgroundColor: '#0033A0', color: '#FFFFFF', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Close</button>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .staff-main-content    { margin-left: 0 !important; padding: 16px !important; padding-top: 72px !important; }
        }
      `}</style>
    </div>
  )
...
function ResponseTable({ responses, formatDate, renderStars, getStatusStyle, onSelect, loading }) {
  if (loading) return <div style={{ padding: '48px', textAlign: 'center', color: '#6c757d' }}>Loading responses...</div>
  if (responses.length === 0) return <div style={{ padding: '48px', textAlign: 'center', color: '#6c757d' }}>No responses found.</div>
  return (
        ...
        <thead>
          <tr style={{ backgroundColor: '#F8F9FA' }}>
            {['Email', 'Client Type', 'Rating', 'Date Submitted', 'Status'].map(h => (
              <th key={h} style={{ padding: '12px 20px', textAlign: 'left', fontSize: '12px', color: '#6c757d', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {responses.map((r, i) => (
            <tr key={r.id || i} style={{ borderBottom: '1px solid #E0E7FF' }} onMouseEnter={e => e.currentTarget.style.backgroundColor='#F5F7FA'} onMouseLeave={e => e.currentTarget.style.backgroundColor='transparent'}>
              <td style={{ padding: '14px 20px' }}>
                <button onClick={() => onSelect(r)} style={{ background: 'none', border: 'none', color: '#0033A0', fontWeight: '600', fontSize: '13px', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>
                  {r.respondent_email || '-'}
                </button>
              </td>
              ...
              <td style={{ padding: '14px 20px' }}>
                <span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, ...getStatusStyle(r.status) }}>{r.status || 'Normal'}</span>
              </td>
            </tr>
          ))}
        </tbody>
...
function FlaggedCard({ response, formatDate, onResolved }) {
  ...
  const handleSubmitResolution = async () => {
    ...
    setSubmitting(true)
    const { error } = await supabase.from('survey_responses').update({ resolution_note: resolutionNote.trim() }).eq('id', response.id)
    setSubmitting(false)
    if (error) { alert('Failed to submit. Please try again.'); return }
    setDone(true)
  ...
  return (
    <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '24px', border: '2px solid #DC2626', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <span style={{ backgroundColor: '#DC2626', color: '#fff', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 700 }}>🚩 Flagged</span>
      </div>
      ...
      {!done && (
        !showResolveBox ? (
          <button onClick={() => setShowResolveBox(true)} style={{ padding: '10px 24px', backgroundColor: '#0033A0', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
            📝 Submit Resolution
          </button>
        ) : (
          <div style={{ marginTop: '8px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#1A1A2E', marginBottom: '8px' }}>Resolution Note <span style={{ color: '#DC2626' }}>*</span></label>
            <textarea placeholder="Describe what actions were taken to resolve this issue..." value={resolutionNote} onChange={e => setResolutionNote(e.target.value)} rows={4} style={{ width: '100%', padding: '12px', border: '2px solid #E0E7FF', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', resize: 'vertical', outline: 'none', marginBottom: '12px', boxSizing: 'border-box' }} />
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => setShowResolveBox(false)} style={{ flex: 1, padding: '10px', backgroundColor: '#F5F7FA', color: '#6c757d', border: '1px solid #E0E7FF', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleSubmitResolution} disabled={!resolutionNote.trim() || submitting} style={{ flex: 2, padding: '10px', backgroundColor: resolutionNote.trim() ? '#0033A0' : '#E0E7FF', color: resolutionNote.trim() ? '#fff' : '#adb5bd', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: resolutionNote.trim() ? 'pointer' : 'not-allowed' }}>
